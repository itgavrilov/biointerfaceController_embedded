...
#define  ADS1256_DRDY_PIN         ADS1256_DRDY_Pin
#define  ADS1256_DRDY_PORT        ADS1256_DRDY_GPIO_Port 

//-------------------------------------
// Предварительно, ввести сюда тайминги настройки частоты Шины SPI из STM32Cube:
#define  STM32_MCU_SYSCLK             48000000                /*частота ядра*/
#define  STM32_MCU_APBCLK             ((STM32_MCU_SYSCLK)/1)  /*частота периферийной шины (половинная)*/
...
#define  ADS1256_F_CLKIN  8000000
...
#define  ADS1256_F_DATA   1000  /*предельный максимум для текущей аппаратной конфигурации, но я не буду его использовать - возьму с запасом, в 2 раза медленнее*/
...
// порта CS (включение интерфейса SPI в удаленном ADS1256 модуле - способность АЦП слушать команды и выдавать ответы)
...
#else
	// порт CS включается в настройках как NSS = SPI_NSS_HARD_OUTPUT
...
//  HAL_Delay(1000);
  ...
  ADS1256_API_SetInputBufferMode( ADS1256_STATUS_BUFEN_OFF );              //DEBUG: нужно включить в реальном изделии
